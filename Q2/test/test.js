...
describe("HelloWorld", function () {
    ...
    it("Should return true for correct proof", async function () {
        ...
        //fullProve(input parameters of the circuit, compiled circuit (WASM), circuit zkey (FINAL))
        //generates the proof and public signals
        const { proof, publicSignals } = await groth16.fullProve({"a":"1","b":"2"}, "contracts/circuits/HelloWorld/HelloWorld_js/HelloWorld.wasm","contracts/circuits/HelloWorld/circuit_final.zkey");

        // Input parameters are 1 and 2
        // publicSignals[0] must be 1*2 = 2
        console.log('1x2 =',publicSignals[0]);

        // Unstringifying bits: converting from string to BigInt
        const editedPublicSignals = unstringifyBigInts(publicSignals);
        const editedProof = unstringifyBigInts(proof);

        //Creating calldata that is suitable for solidity smart contract call
        const calldata = await groth16.exportSolidityCallData(editedProof, editedPublicSignals);
    
        //Proper format
        const argv = calldata.replace(/["[\]\s]/g, "").split(',').map(x => BigInt(x).toString());
    
        // Proper format and values for right proof
